#!/usr/bin/env python3
"""
Comprehensive test: preferred and staged semantics enumeration vs optN consistency
"""
import subprocess
import sys
...
def run_semantics(semantic, framework, use_optN=False, timeout=30):
    """Run a semantics on a framework"""
...
def test_framework(semantic, framework, timeout=30):
    """Test a single framework"""
    enum_exts, enum_err = run_semantics(semantic, framework, use_optN=False, timeout=timeout)
    optN_exts, optN_err = run_semantics(semantic, framework, use_optN=True, timeout=timeout)
...
def test_semantic(semantic_name):
    """Test a semantic on all frameworks"""
    print("=" * 80)
    print(f"{semantic_name.upper()} Semantics Consistency Test: Enumeration vs OptN")
    print("=" * 80)
    ...
    for i, framework in enumerate(test_files, 1):
        status, msg = test_framework(semantic_name, framework, timeout=10)
        
        name = framework[framework.find('/')+1:]
...
def main():
    ...
    for semantic in ['preferred', 'staged']:
        passed, failed, skipped = test_semantic(semantic)
        results[semantic] = (passed, failed, skipped)
        print("\n")
    ...
    print("=" * 80)
    for semantic, (passed, failed, skipped) in results.items():
        status = "✓ PASS" if failed == 0 else "✗ FAIL"
        print(f"{semantic.upper():12s}: {status} - {passed} passed, {failed} failed, {skipped} skipped")
    print("=" * 80)
    
    total_failed = sum(f for _, f, _ in results.values())
