#!/usr/bin/env python3
"""
WABA Full Validation Benchmark Runner

Side-by-side comparison of three modes:
- enum-old: Old base + old semirings + old monoids + enumeration
- enum-new: New base + new semirings + old monoids + enumeration
- new-opt: New base + new semirings + new monoids + optimization (optN, both directions)

Total: 80 configurations × 120 frameworks = 9,600 runs
Expected time: ~13-15 days with 1 worker at 120s timeout
"""
...
from config import (
    get_full_validation_configurations,
...
def get_results_dir(run_id: str) -> Path:
    """Get results directory for full validation run."""
    results_dir = BENCHMARK_ROOT / "results" / "full_validation" / run_id
...
def schedule_benchmarks(frameworks: List[Path], configs: List[Dict],
    ...
    print(f"\n{'='*80}")
    print(f"FULL VALIDATION BENCHMARK")
    print(f"{'='*80}")
        ...
        for future in as_completed(future_to_task):
            ...
            try:
                ...
                if result.get('status') == 'TIMEOUT':
                    ...
                    errors += 1
                    outcome = "✗ ERROR"
                elif result.get('unsatisfiable', False):
                    unsatisfiable += 1
                    outcome = "∅ UNSAT"
                elif result.get('optimum_found', False):
                    outcome = f"✓ OPTIMUM"
                elif result.get('models_found', 0) > 0:
                    outcome = f"✓ {result['models_found']} models"
                else:
                    outcome = "? UNKNOWN"
            ...
            if completed % 100 == 0:
                ...
                print(f"Timeouts: {timeouts}, Errors: {errors}, Unsatisfiable: {unsatisfiable}")
                print()
...
def main():
    parser = argparse.ArgumentParser(
        description='WABA Full Validation Benchmark Runner',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Run full validation benchmark (1 worker, 120s timeout)
  python3 run_full_validation.py --timeout 120 --workers 1

  # Test on 5 frameworks per topology
  python3 run_full_validation.py --timeout 120 --workers 1 --limit 5

  # Resume interrupted run
  python3 run_full_validation.py --timeout 120 --workers 1 --resume --run-id 20231201_120000
        """
    )
    ...
    configs = get_full_validation_configurations()

    print(f"\nFrameworks discovered: {len(frameworks)}")
    print(f"Configurations: {len(configs)}")
    print(f"  - old-enum: {len([c for c in configs if c['mode'] == 'old-enum'])}")
    print(f"  - new-enum: {len([c for c in configs if c['mode'] == 'new-enum'])}")
    print(f"  - old-opt: {len([c for c in configs if c['mode'] == 'old-opt'])}")
    print(f"  - new-opt: {len([c for c in configs if c['mode'] == 'new-opt'])}")
